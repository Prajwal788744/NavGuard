...
const Profile = () => {
  ...
  const [form, setForm] = useState({ name: '', phone: '', nationality: '', emergencyContact: '', bloodGroup: '', gender: '', dob: '' });

  useEffect(() => {
    if (!user) { navigate('/login'); return; }
    setForm({ name: user.name || '', phone: user.phone || '', nationality: user.nationality || '', emergencyContact: user.emergencyContact || '', bloodGroup: user.bloodGroup || '', gender: user.gender || '', dob: user.dob || '' });
  }, [user]);
      ...
      <main className="container max-w-lg mx-auto px-4 py-6">
        <motion.div className="glass-card glow-border p-6 space-y-5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div>
              <label className="text-xs text-muted-foreground font-medium mb-1 block">Gender</label>
              <select
                value={form.gender}
